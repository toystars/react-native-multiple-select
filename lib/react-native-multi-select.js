...
export default class MultiSelect extends Component {
  ...
  _addItem = () => {
    ...
    let newItemName = this.state.searchTerm;
    if (newItemName) {
      let newItemId = newItemName
        .split(' ')
      ...
      >
        <View>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            >
              Add {item.name} (press return or click)
            </Text>
          </View>
        </View>
      </TouchableOpacity>
  ...
  _renderItems = () => {
    ...
    // element to the list.
    // Also, if match, return should maybe select the first item?
    let searchTermMatch;
    let itemList;
    let addItemRow;
    ...
    if (!searchTermMatch && searchTerm.length) {
